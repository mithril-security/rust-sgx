...
extern "C" fn handle_signal(signo: c_int, _info: *mut libc::siginfo_t, context: *mut c_void) {
    ...
    unsafe {
        ...
        match (insn_is_enclu, insn_in_enclave) {
                        ...
                        find_enclave_by_address(&mut SIMULATED_ENCLAVES.lock(), rbx)
                    {
                        if let Some(tcsstate) = enclave.tcss.get_mut(&rbx) {
                            ...
                            *tcsstate = TcsState::Entered { oldgs, aep: rcx };
                            ...
                            context.uc_mcontext.gregs[Greg::RCX as usize] =
                                (rip + ENCLU.len() as u64) as _;
                            context.uc_mcontext.gregs[Greg::RIP as usize] =
                                (enclave.base + tcs.oentry) as _;
                            ...
                            panic!(
                                "Tried to enter invalid TCS in simulated enclave: 0x{:x}",
                                rbx
                            );
                        }
                    }
            ...
            (true, Some(base)) => {
                ...
                if rax == Enclu::EExit as i64 {
                    let tcs = match IN_TCS.with(|in_tcs| in_tcs.take()) {
                        Some(tcs) => tcs.get(),
                        None => panic!(
                            "ENCLU[EEXIT] called while OS thread not in TCS (RIP=0x{:x})",
                            rip
                        ),
                    };
                    ...
                    let (oldgs, aep) = match enclave.tcss[&tcs] {
                        ...
                        TcsState::NotEntered => panic!(
                            "Tried to exit TCS that wasn't entered in simulated enclave: 0x{:x}",
                            tcs
                        ),
                    };
                    *enclave.tcss.get_mut(&tcs).unwrap() = TcsState::NotEntered;
                    ...
                    context.uc_mcontext.gregs[Greg::RIP as usize] =
                        context.uc_mcontext.gregs[Greg::RBX as usize];
            ...
            (false, Some(_)) => {
                unimplemented!(
                    "simulate AEX for TCS 0x{:x}",
                    IN_TCS.with(|in_tcs| in_tcs.get()).unwrap().get()
                )
            }
        }
